...
//!
//! - default or `"chrono"` feature will use the [`chrono`](https://crates.io/crates/chrono) crate
//! - `"time"` feature will use the [`time`](https://crates.io/crates/time) crate
...
pub use dates::{PlainDate, Weekday};
pub use divisions::Division;
pub use errors::Error;
pub use work_days::{MonToFriWorkDays, WorkDays};
...
/// ```
pub mod prelude {
    pub use super::{BankHolidayCalendar, Division, PlainDate};
}
...
mod tests {
    ...
    #[cfg(feature = "chrono")]
    type Date = dates::chrono::DateImpl;
    #[cfg(all(not(feature = "chrono"), feature = "time"))]
    type Date = dates::time::DateImpl;

    fn test(calendar: BankHolidayCalendar<Date, MonToFriWorkDays>) {
    ...
    async fn custom_work_days() {
        ...
        impl WorkDays<Date> for PartTime {
        ...
        let work_days_february_2024 = std::iter::from_fn(move || {
            ...
            date = PlainDate::next_day(&date);
            Some(is_work_day)
        })
        .take(29);
        let expected = [false, false, false, false, true, true, true]
            .iter()
            .copied()
            .cycle()
            .take(29);
    ...
    fn next_holiday() {
        ...
        let mut holidays = calendar
            .iter_holidays_after(&date, None)
            .map(|holiday| holiday.date().as_components());
        assert_eq!(holidays.next(), Some((2016, 3, 25)));
        assert_eq!(holidays.next(), Some((2016, 5, 2)));
        assert_eq!(holidays.next(), Some((2016, 5, 30)));

        let mut holidays = calendar
            .iter_holidays_after(&date, Some(Division::Scotland))
            .map(|holiday| holiday.date().as_components());
    ...
    fn previous_holiday() {
        ...
        let mut holidays = calendar
            .iter_holidays_before(&date, None)
            .map(|holiday| holiday.date().as_components());
        ...
        let mut holidays = calendar
            .iter_holidays_before(&date, Some(Division::Scotland))
            .map(|holiday| holiday.date().as_components());
        assert_eq!(holidays.next(), Some((2016, 1, 4)));
        assert_eq!(holidays.next(), Some((2016, 1, 1)));
        assert_eq!(holidays.next(), Some((2015, 12, 28)));
    ...
    fn holiday_check() {
        let calendar = BankHolidayCalendar::cached();
        let date = Date::try_from_components(2012, 1, 2).unwrap();
        let is_holiday = calendar.is_holiday(&date, None);
        assert!(is_holiday);
        let date = Date::try_from_components(2016, 1, 4).unwrap();
        let is_holiday = calendar.is_holiday(&date, None);
        assert!(!is_holiday);
        let date = Date::try_from_components(2016, 1, 4).unwrap();
        let is_holiday = calendar.is_holiday(&date, Some(Division::Scotland));
        assert!(is_holiday);
    }
    ...
    fn next_work_day() {
        ...
        let mut work_days = calendar
            .iter_work_days_after(Date::try_from_components(2017, 12, 19).unwrap(), None)
            .map(|date| date.as_components());
        ...
        let mut work_days = calendar
            .iter_work_days_after(
                Date::try_from_components(2017, 12, 19).unwrap(),
                Some(Division::Scotland),
            )
            .map(|date| date.as_components());
        assert_eq!(work_days.next(), Some((2017, 12, 20)));
        assert_eq!(work_days.next(), Some((2017, 12, 21)));
    ...
    fn previous_work_day() {
        ...
        let mut work_days = calendar
            .iter_work_days_before(Date::try_from_components(2018, 1, 3).unwrap(), None)
            .map(|date| date.as_components());
        assert_eq!(work_days.next(), Some((2018, 1, 2)));
        assert_eq!(work_days.next(), Some((2017, 12, 29)));
        assert_eq!(work_days.next(), Some((2017, 12, 28)));
        ...
        let mut work_days = calendar
            .iter_work_days_before(Date::try_from_components(2018, 1, 3).unwrap(), Some(Division::Scotland))
            .map(|date| date.as_components());
        assert_eq!(work_days.next(), Some((2017, 12, 29)));
        assert_eq!(work_days.next(), Some((2017, 12, 28)));
        assert_eq!(work_days.next(), Some((2017, 12, 27)));
    ...
    fn holidays_2018_to_2022(bank_holiday: &BankHoliday<Date>) -> bool {
    ...
    fn number_of_bank_holidays() {
        ...
        for (division, expected_count) in expectation {
            let holiday_count = calendar
                .holidays(division)
                .into_iter()
                .filter(|holiday| holidays_2018_to_2022(holiday))
                .count();
            assert_eq!(
                holiday_count, expected_count,
                "Unexpected number of bank holidays in {division:?}"
            );

            let end_of_2017 = Date::try_from_components(2017, 12, 31).unwrap();
            let holiday_count = calendar
                .iter_holidays_after(&end_of_2017, division)
                .filter(|holiday| holidays_2018_to_2022(holiday))
                .count();
            assert_eq!(
                holiday_count, expected_count,
            ...
            let holiday_count = calendar
                .iter_holidays_before(&start_of_2023, division)
                .filter(|holiday| holidays_2018_to_2022(holiday))
                .count();
            assert_eq!(
                holiday_count, expected_count,
    ...
    fn bank_holidays_in_divisions() {
        ...
        for (division, title, expect_exists) in expectation {
            let bank_holiday_found = calendar
                .holidays(division)
                .iter()
                .filter(|holiday| holidays_2018_to_2022(holiday))
                .any(|holiday| holiday.title() == title);
            assert_eq!(
                bank_holiday_found, expect_exists,
                "Expect “{title}” to exist in {division:?}: {expect_exists}"
            );
        }
    }
}
